...
# ---------------------------------------------------------
# HELPER: UTF-8 without BOM file creation
# ---------------------------------------------------------
# WHY: Cross-platform compatibility (Linux containers, Git)
...
Get-ChildItem -Filter *.sln -ErrorAction SilentlyContinue | Remove-Item -Force

Write-Host "  ✓ Cleanup complete`n" -ForegroundColor Green

# ---------------------------------------------------------
# STEP 2: PROJECT STRUCTURE (Onion Architecture)
# ---------------------------------------------------------
Write-Host "🏗️  Creating Onion Architecture structure..." -ForegroundColor Green

# Create solution
dotnet new sln -n $SlnName --force | Out-Null
...
public class DemoAgentService : IAgentService
{
    public Task<ChatSession> ProcessRequestAsync(string userPrompt)
    {
        var session = new ChatSession
        {
            ...
            FinalAnswer = "⚠️ AI is not configured. Set OPENAI_API_KEY environment variable to enable Writer-Critic AI agent.",
            Iterations = 0
        };
...
<Project Sdk="Microsoft.NET.Sdk.Web">
  ...
  <ItemGroup>
    <!-- Ensure CSV is copied to output directory -->
...
Royal Canin Maxi Adult 15kg;249.99;Karma Sucha
Brit Care Lamb & Rice 12kg;189.50;Karma Sucha
Dolina Noteci Puszka 800g;12.90;Karma Mokra
Pedigree Dentastix;15.00;Przysmaki
Purina Pro Plan Medium;210.00;Karma Sucha
Obroża GPS Wodoodporna;299.00;Akcesoria
Smycz Automatyczna 5m;45.00;Akcesoria
Piłeczka Gumowa;15.50;Zabawki
Kong Classic Large;69.00;Zabawki
...
New-File "src/$SlnName.WebUI/_Imports.razor" @"
...
<head>
    ...
    <base href="/" />
    <title>PetWorld AI - Intelligent Sales Assistant</title>
...
New-File "src/$SlnName.WebUI/Components/Routes.razor" @'
...
<div class="container-fluid mt-4">
    <div class="row">
        ...
        <div class="col-lg-8">
            <div class="card shadow-sm">
                <div class="card-header bg-primary text-white">
                    <h5 class="mb-0">🐾 PetWorld AI Assistant</h5>
                    <small>Powered by Writer-Critic Pattern (Hallucination Prevention)</small>
                </div>
                <div class="card-body">
                    <div class="input-group mb-3">
                        <input @bind="Input"
                               class="form-control"
                               placeholder="Ask about products (e.g., 'What food do you recommend for senior cats?')"
                               @onkeyup="HandleKeyUp"
                               disabled="@IsLoading" />
                        <button @onclick="Ask"
                                class="btn btn-dark"
                    ...
                    @if (IsLoading)
                    {
                        <div class="text-center my-4">
                            ...
                            <p class="text-muted mt-2">Writer-Critic loop in progress...</p>
                        </div>
                    }
                    ...
                    @if (Session != null)
                    {
                        <div class="alert alert-light border">
                            <strong>Your Question:</strong> @Session.UserQuestion
                        </div>
                        ...
                        <!-- Transparency: Show iteration logs -->
                        <details class="mt-3">
                            <summary class="text-muted" style="cursor: pointer;">
                                🔍 View AI Decision Process (@Session.Iterations iterations)
                            </summary>
                            <ul class="list-group mt-2">
                                @foreach (var log in Session.Logs)
                                {
                                    <li class="list-group-item">
                                        <span class="badge @(log.AgentRole == "Critic" ? "bg-danger" : "bg-info")">
                                            @log.AgentRole
                                        </span>
                                        <small class="d-block mt-1">@log.Content</small>
                                    </li>
                                }
                            </ul>
                        </details>
        ...
        <!-- RIGHT: Live Product Inventory -->
        <div class="col-lg-4">
            <div class="card shadow-sm">
                <div class="card-header bg-success text-white">
                    <h6 class="mb-0">📦 Live Product Catalog</h6>
                    <small>AI uses this data to prevent hallucinations</small>
                </div>
                    ...
                    else
                    {
                        @foreach (var p in Products)
                        {
                            <div class="list-group-item d-flex justify-content-between align-items-start">
                                ...
                                <span class="badge bg-secondary rounded-pill">@p.Price.ToString("F2") PLN</span>
                            </div>
                        }
                    }
                </div>
                <div class="card-footer d-flex justify-content-between align-items-center">
                    <button class="btn btn-sm btn-outline-success"
                        ...
                        @(IsLoadingProducts ? "Refreshing..." : "Refresh Catalog")
                    </button>
...
OPENAI_API_KEY=zmien_mnie_w_pliku_env

# Database Connection (used by Docker Compose)
...
**Built with ❤️ for AI-Native .NET Developer recruitment task**
'@
...
I didn't blindly accept AI suggestions. Each recommendation was:
...
Write-Host "  ✓ Documentation created`n" -ForegroundColor Green
...
Write-Host "   OPENAI_API_KEY=your_actual_key_here`n" -ForegroundColor Gray
...
Write-Host "   - .env.example           (Configuration template)`n" -ForegroundColor White
...
Write-Host "   These explain the WHY behind technical decisions.`n" -ForegroundColor Gray
